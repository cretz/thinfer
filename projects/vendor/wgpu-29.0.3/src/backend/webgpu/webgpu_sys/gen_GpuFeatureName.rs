...
pub enum GpuFeatureName {
    ...
    // THINFER-PATCH(web-subgroups #5555): upstream's vendored web-sys bindings
    // omit the "subgroups" GPUFeatureName, so wgpu's web backend can neither
    // report nor request it. Add it so we can enable subgroup builtins on web.
    Subgroups = "subgroups",
}
