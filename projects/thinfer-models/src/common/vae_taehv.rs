//! TAEHV tiny video decoder (madebyollin TAEHV family), the model-agnostic
//! `VaeChoice::Tiny` fast path shared by Wan (`lighttaew2_2`) and HunyuanVideo
//! (`taehv1_5`). It trades the real conv3d VAE's bandwidth (the run's dominant
//! cost) for a ~0.4GB per-frame 2D-conv stack; the real VAE stays the parity
//! default. The decoder graph is identical across these models -- only the
//! latent channel count (`z_dim`) and the host latent normalization differ,
//! both carried by [`TaehvConfig`].
//!
//! Decode (batched-over-frames): a latent-frame batch `[T, z_dim, H, W]` (N=1)
//! pushed through a flat block sequence:
//!   conv z_dim->256 -> ReLU -> 3x MemBlock@256
...
//!
//! Host-side (off the GPU graph): denorm `z*std+mean` (per-channel; identity for
//! models trained without a latent scale) + Clamp `tanh(x/3)*3` before upload;
//! clamp[0,1] + pixel_shuffle(2) + trim first 3 frames + `*2-1` after readback.
//! So only conv2d/relu/memcat/upsample dispatch on-device.
...
/// Geometry + latent normalization a [`TaehvDecoder`] needs. The decoder graph
/// (STAGE_C `[256,128,64]`, TGrow strides `[1,2,2]`, 16x spatial / 4x temporal,
/// conv_out 64->12) is fixed across the TAEHV family; only these per-model
/// fields vary.
#[derive(Clone, Debug)]
pub struct TaehvConfig {
    /// Latent channels = decoder `conv_in` input channels (Wan 48, Hunyuan 32).
    pub z_dim: usize,
    /// Trailing pixel-shuffle factor (2 -> 16x spatial with the 8x conv stack).
    pub patch_size: usize,
    /// Total spatial compression (16).
    pub spatial_compression: usize,
    /// Per-channel latent denorm applied before the `tanh(x/3)*3` clamp:
    /// `z*std+mean`. Models trained without a latent scale (Hunyuan TAEHV
    /// consumes raw latents) pass `std=1, mean=0` (length `z_dim` each).
...
#[derive(Clone, Debug)]
pub struct TaehvMemBlock {
    pub conv0: TaehvConv, // 2C -> C
    pub conv2: TaehvConv, // C -> C
    pub conv4: TaehvConv, // C -> C
}
...
#[derive(Clone, Debug)]
pub struct TaehvDecoderWeights {
    pub conv_in: TaehvConv, // idx1  48 -> 256 (+bias)
    pub mb: [[TaehvMemBlock; 3]; 3],
    pub tgrow: [TaehvConv; 3], // idx7/13/19 .conv (no bias)
    pub inter: [TaehvConv; 3], // idx8/14/20 (no bias)
    pub conv_out: TaehvConv,   // idx22 64 -> 12 (+bias)
}

fn conv_id(idx: usize, bias: bool) -> TaehvConv {
    TaehvConv {
...
fn memblock_ids(idx: usize) -> TaehvMemBlock {
    let c = |sub: usize| TaehvConv {
...
fn tgrow_id(idx: usize) -> TaehvConv {
    TaehvConv {
...
impl Default for TaehvDecoderWeights {
    fn default() -> Self {
        Self::new()
    }
}

impl TaehvDecoderWeights {
    pub fn new() -> Self {
        Self {
...
pub struct TaehvConvHandles {
    pub weight: WeightHandle,
    pub bias: Option<WeightHandle>,
}

#[derive(Clone, Copy, Debug)]
pub struct TaehvMemBlockHandles {
    pub conv0: TaehvConvHandles,
    pub conv2: TaehvConvHandles,
    pub conv4: TaehvConvHandles,
}

#[derive(Clone, Debug)]
pub struct TaehvDecoderHandles {
    pub conv_in: TaehvConvHandles,
    pub mb: [[TaehvMemBlockHandles; 3]; 3],
    pub tgrow: [TaehvConvHandles; 3],
    pub inter: [TaehvConvHandles; 3],
    pub conv_out: TaehvConvHandles,
}

fn reg_conv<S: WeightSource>(
    residency: &WeightResidency<S>,
    w: &TaehvConv,
) -> Result<TaehvConvHandles, LoadError> {
    Ok(TaehvConvHandles {
...
fn reg_memblock<S: WeightSource>(
    residency: &WeightResidency<S>,
    w: &TaehvMemBlock,
) -> Result<TaehvMemBlockHandles, LoadError> {
    Ok(TaehvMemBlockHandles {
...
pub fn register_decoder_taehv<S: WeightSource>(
    residency: &WeightResidency<S>,
    w: &TaehvDecoderWeights,
) -> Result<TaehvDecoderHandles, LoadError> {
    let mb = |s: usize| -> Result<[TaehvMemBlockHandles; 3], LoadError> {
        Ok([
    ...
    let conv = |c: &TaehvConv| reg_conv(residency, c);
    Ok(TaehvDecoderHandles {
...
pub struct TaehvConvViews<'a> {
    pub weight: GpuView<'a>,
    pub bias: Option<GpuView<'a>>,
}
pub struct TaehvMemBlockViews<'a> {
    pub conv0: TaehvConvViews<'a>,
    pub conv2: TaehvConvViews<'a>,
    pub conv4: TaehvConvViews<'a>,
}
pub struct TaehvDecoderViews<'a> {
    pub conv_in: TaehvConvViews<'a>,
    pub mb: [[TaehvMemBlockViews<'a>; 3]; 3],
    pub tgrow: [TaehvConvViews<'a>; 3],
    pub inter: [TaehvConvViews<'a>; 3],
    pub conv_out: TaehvConvViews<'a>,
}

/// A conv's weight + (optional) bias BufRefs. `bias` is `None` for the
/// bias-less convs; the forward substitutes a shared zero buffer.
#[derive(Clone, Copy, Debug)]
pub struct TaehvConvBufs {
    pub weight: BufRef,
    pub bias: Option<BufRef>,
}
#[derive(Clone, Copy, Debug)]
pub struct TaehvMemBlockBufs {
    pub conv0: TaehvConvBufs,
    pub conv2: TaehvConvBufs,
    pub conv4: TaehvConvBufs,
}
#[derive(Clone, Debug)]
pub struct TaehvDecoderBufs {
    pub conv_in: TaehvConvBufs,
    pub mb: [[TaehvMemBlockBufs; 3]; 3],
    pub tgrow: [TaehvConvBufs; 3],
    pub inter: [TaehvConvBufs; 3],
    pub conv_out: TaehvConvBufs,
}

impl TaehvConvViews<'_> {
    fn bufs(&self) -> TaehvConvBufs {
        TaehvConvBufs {
...
impl TaehvMemBlockViews<'_> {
    fn bufs(&self) -> TaehvMemBlockBufs {
        TaehvMemBlockBufs {
...
impl TaehvDecoderViews<'_> {
    fn bufs(&self) -> TaehvDecoderBufs {
        let mb_stage = |s: &[TaehvMemBlockViews; 3]| [s[0].bufs(), s[1].bufs(), s[2].bufs()];
        TaehvDecoderBufs {
...
impl TaehvConvHandles {
    async fn acquire<'r, S: WeightSource>(
        &self,
        residency: &'r WeightResidency<S>,
        backend: &WgpuBackend,
    ) -> Result<TaehvConvViews<'r>, ResidencyError<S::Error, WgpuError>> {
        Ok(TaehvConvViews {
...
impl TaehvMemBlockHandles {
    async fn acquire<'r, S: WeightSource>(
        &self,
        residency: &'r WeightResidency<S>,
        backend: &WgpuBackend,
    ) -> Result<TaehvMemBlockViews<'r>, ResidencyError<S::Error, WgpuError>> {
        Ok(TaehvMemBlockViews {
...
async fn acquire_stage<'r, S: WeightSource>(
    s: &[TaehvMemBlockHandles; 3],
    residency: &'r WeightResidency<S>,
    backend: &WgpuBackend,
) -> Result<[TaehvMemBlockViews<'r>; 3], ResidencyError<S::Error, WgpuError>> {
    Ok([
...
async fn acquire_trio<'r, S: WeightSource>(
    s: &[TaehvConvHandles; 3],
    residency: &'r WeightResidency<S>,
    backend: &WgpuBackend,
) -> Result<[TaehvConvViews<'r>; 3], ResidencyError<S::Error, WgpuError>> {
    Ok([
        s[0].acquire(residency, backend).await?,
        s[1].acquire(residency, backend).await?,
        s[2].acquire(residency, backend).await?,
...
impl TaehvDecoderHandles {
    async fn acquire<'r, S: WeightSource>(
        &self,
        residency: &'r WeightResidency<S>,
        backend: &WgpuBackend,
    ) -> Result<TaehvDecoderViews<'r>, ResidencyError<S::Error, WgpuError>> {
        Ok(TaehvDecoderViews {
...
/// targeted.
const TAEHV_CONV_TILE: Conv2dConfig = Conv2dConfig::DEFAULT;

pub struct TaehvPipelines {
...
impl TaehvPipelines {
    pub async fn compile(backend: &WgpuBackend) -> Result<Self, WgpuError> {
        ...
        let conv2d_op = Conv2dF32::new(TAEHV_CONV_TILE);
        Ok(Self {
            act_dtype,
            ...
            upsample: backend
                ...
                .await?,
            add: backend
                .create_pipeline("vae_taehv_add", AddF32::wgsl(cfg), "main", AddF32::layout())
                .await?,
        })
    }
...
impl From<WgpuError> for TaehvError {
...
const TAEHV_SUBMIT_BUDGET_BYTES: u64 = 128 * 1024 * 1024;
...
const TAEHV_WEIGHT_RESERVE: u64 = 512 * 1024 * 1024;
...
fn plan_chunk(
    ...
    let workspace_budget = eff_budget.saturating_sub(TAEHV_WEIGHT_RESERVE);
    let per_lat_frame = (TAEHV_PEAK_K * h_in as u64 * w_in as u64 * asz).max(1);
    let cf = (workspace_budget / per_lat_frame).max(1);
    cf.min(f.max(1) as u64) as usize
}

pub struct TaehvDecoder {
    pub pipelines: TaehvPipelines,
    pub handles: TaehvDecoderHandles,
    pub cfg: TaehvConfig,
}

#[derive(Debug)]
pub enum TaehvDecodeError<SE: core::fmt::Debug> {
    Forward(TaehvError),
    Residency(ResidencyError<SE, WgpuError>),
}

impl<SE: core::fmt::Debug> From<TaehvError> for TaehvDecodeError<SE> {
    fn from(e: TaehvError) -> Self {
        Self::Forward(e)
    }
}
impl<SE: core::fmt::Debug> From<WgpuError> for TaehvDecodeError<SE> {
    fn from(e: WgpuError) -> Self {
        Self::Forward(TaehvError::Wgpu(e))
    }
}
impl<SE: core::fmt::Debug> From<ResidencyError<SE, WgpuError>> for TaehvDecodeError<SE> {
...
impl TaehvDecoder {
    ...
    pub async fn decode<S: WeightSource>(
    ...
    ) -> Result<Vec<f32>, TaehvDecodeError<S::Error>> {
        let cfg = &self.cfg;
        ...
        if budget != u64::MAX {
            residency.set_transient_reserve(budget.saturating_sub(TAEHV_WEIGHT_RESERVE));
        }
        ...
        for chunk_idx in 0..n_chunks {
            ...
            {
                let mut packer = ScopePacker::new(workspace, TAEHV_SUBMIT_BUDGET_BYTES);
